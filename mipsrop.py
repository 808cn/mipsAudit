...
class MIPSROPFinder(object):
    ...
    def __init__(self):
        ...
        if self.controllable_jumps or self.system_calls:
            print ("MIPS ROP Finder activated, found %d controllable jumps between 0x%.8X and 0x%.8X\nMIPS ROP Help : mipsrop.help()" % (len(self.controllable_jumps), self.start, self.end))
        else:
    ...
    def _get_instruction(self, ea):
        return MIPSInstruction(idc.print_insn_mnem(ea), idc.print_operand(ea, 0), idc.print_operand(ea, 1), idc.print_operand(ea, 2), ea)
    ...
    def _find_system_calls(self, start_ea, end_ea):
        ...
        for xref in idautils.XrefsTo(idc.get_name_ea_simple('system')):
            ea = xref.frm
            mnem = idc.print_insn_mnem(ea)
